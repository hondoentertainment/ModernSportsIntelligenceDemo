...
**Multi-agent production audit (follow-up):** Parallel security/CI/DAL reviews drove code changes (not only docs): (1) **eBay** — removed `action: token` that exposed OAuth `accessToken` to browsers; replaced with `verify` returning `{ ok: true }` only after server-side token fetch. (2) **`api/lib/httpProduction.ts`** — shared CORS: **`ALLOWED_ORIGIN`** or **`https://$VERCEL_URL`**; **`*` only in non-production**; production without a resolved origin omits `Access-Control-Allow-Origin` and logs a warning. **`Access-Control-Allow-Headers`** includes **`Authorization`** for Bearer JWT preflight. Generic **`500`** JSON (`code` + safe message; full error server-logged only). (3) **`initDAL`** — deduped by `userId|demo|live` and **`store.clear()`** on real user/mode changes to reduce stale in-memory cache when switching accounts. (4) **`useSupabaseInventory`** — load **epoch** so slower cloud fetches cannot overwrite newer user/session results; cloud fetch failures call **`markCloudFailure`**. (5) **`GlobalErrorBoundary`** — calls **`reportError`** (Sentry + beacon) like other boundaries.

**Phase 15 (API CORS + prod shell tests):** `setApiCorsHeaders` production hardening and **`Authorization`** on preflight; **`isClientProductionBuild()`** in `lib/utils/env.ts` for **`ProductionConfigBanner`** (RTL tests in `tests/components/ProductionConfigBanner.test.ts`, `tests/api/httpProduction.test.ts`).

**Phase 16 (rollout orchestration):** Master checklist **[docs/PRODUCTION_ROLLOUT_PHASES.md](docs/PRODUCTION_ROLLOUT_PHASES.md)**; Edge Function JWT contract **[docs/SUPABASE_EDGE_FUNCTIONS.md](docs/SUPABASE_EDGE_FUNCTIONS.md)**; **`npm run test:e2e:deployed`** against Vercel; **`npm run audit:localstorage`** for DAL prioritization; optional CI job when `ENABLE_DEPLOYED_E2E` + `PLAYWRIGHT_DEPLOYMENT_URL` are set.
...
- **Phase 9 – Security:** Enforcing `Content-Security-Policy` in `vercel.json` with violation reporting (`report-uri`/`report-to` → `api/csp-report.ts`); Zod body validation in `api/ai/generate.ts`; per-IP rate limits on `api/ai/generate.ts` and `api/market/ebay.ts` (`api/lib/rateLimit.ts`, env in `.env.example`); details in `docs/MONITORING.md`.
...
- **PWA / offline (v4 hardening):** `components/PwaUpdateBanner.tsx` listens for `sw-update-available` (from `index.html`) and reloads after `SKIP_WAITING`; service worker cache generation bumped in `public/sw.js` when shell caching behavior changes. `public/offline.html` offers HashRouter recovery link `/#/` and branded shell. The enforcing CSP in `vercel.json` includes **`wss:`** for Supabase Realtime, **`js.stripe.com`** / **`hooks.stripe.com`** / **`checkout.stripe.com`** for Stripe, **`worker-src`**, **`img-src`** (`data:` / `blob:` / `https:`), **`base-uri`** / **`frame-ancestors`**, and `report-uri`/`report-to` for violation reporting (see [docs/CSP_ROLLOUT.md](docs/CSP_ROLLOUT.md)).
