...
- **Audit trail UI surface (user timeline viewer):** `pages/AuditTrail.tsx`
  ...
  and CSV export. RFC 4180-quoted CSV is generated by
  `exportAuditEventsToCSV()` in `lib/utils/auditTrailService.ts`.
...
- Admin-grade audit timeline (cross-user, role-gated) for support and
  compliance staff — current viewer is correctly scoped to the signed-in user.
- Pagination + "load older" beyond the 200-row default once any single user
  generates enough events to need it.
- Key rotation automation and secret revocation runbook execution drill.
