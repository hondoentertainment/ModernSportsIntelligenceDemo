...
export function exportPrepListHTML(eventId: string, cards: CardInventory[]): string {
  ...
  const rows = prepList.items.map(item => {
    ...
    if (!card) return '';
    const condOrGrade = card.isGraded
      ? `${card.gradingCompany ?? ''} ${card.grade ?? ''}`.trim()
      : String(card.condition ?? '');
    return `
      <tr>
        <td style="padding:8px;border-bottom:1px solid #333;">${escapeHtml(card.player)}</td>
        <td style="padding:8px;border-bottom:1px solid #333;">${escapeHtml(card.year)} ${escapeHtml(card.manufacturer ?? '')} ${escapeHtml(card.set)} #${escapeHtml(card.cardNumber ?? '')}</td>
        <td style="padding:8px;border-bottom:1px solid #333;">${escapeHtml(condOrGrade)}</td>
        <td style="padding:8px;border-bottom:1px solid #333;">$${escapeHtml(card.purchasePrice.toFixed(2))}</td>
        <td style="padding:8px;border-bottom:1px solid #333;font-weight:bold;">$${escapeHtml(item.targetSalePrice.toFixed(2))}</td>
        <td style="padding:8px;border-bottom:1px solid #333;">${escapeHtml(item.notes ?? '')}</td>
        <td style="padding:8px;border-bottom:1px solid #333;">${escapeHtml(item.status)}</td>
      </tr>
    `;
  }).filter(Boolean).join('');
...
<html>
<head>
  <title>${escapeHtml(event.name)} - Prep List</title>
  <style>
...
<body>
  ...
  <h2>${escapeHtml(event.date)} | ${escapeHtml(event.location)} | ${escapeHtml(prepList.items.length)} cards</h2>
  <table>
    <thead>
      <tr>
        <th>Player</th>
  ...
  <div class="footer">Generated by MSI on ${escapeHtml(new Date().toLocaleDateString())}</div>
</body>
</html>`;
}
