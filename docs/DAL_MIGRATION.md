...
This document ties together the production persistence stack introduced under **[PRODUCTION_READINESS.md §1.1](../PRODUCTION_READINESS.md#11-replace-localstorage-with-real-backend-services)** (_Replace localStorage with Real Backend Services_).
...
| **2 — Billing & account**     | Stripe-adjacent state, auth history, compliance                 | **Canonical:** `lib/utils/authenticationService.ts`, `lib/utils/taxHarvestService.ts`, `lib/utils/hobbyIncomeService.ts`, `lib/utils/complianceCenterService.ts` (all use **`store`**). Root **`lib/authenticationService.ts`**, **`lib/taxHarvestService.ts`**, etc. re-export utils — import either path. **Batch 2 shipped:** `lib/utils/syncScheduler.ts` uses **`msi_sync_scheduler_config`** / **`msi_sync_scheduler_schedule`** (one-time migration from `cardx_sync_*` via **`migrateLocalStorageJsonKeyOnce`** in `lib/dal/syncStore.ts`). **`lib/utils/useAlerts.ts`** uses **`msi_alerts`** (migration from `cardx_alerts`). **`listLocalStorageKeysWithPrefix`** centralizes enumeration for **`lib/utils/userStatePersistence.ts`** (prune/clear) and **`lib/utils/offlineService.ts`** (storage stats). |
...
Large root files that already have a same-named twin under `lib/utils`, `lib/analytics`, `lib/core`, `lib/trading`, or `lib/social` are replaced with a **thin re-export** so there is a single canonical implementation. Batch 3 covered trading-heavy names; **wave 2** barreled 45 modules (e.g. `realTimePriceEngineService`, `gemini`, `socialService`). **Waves 3–4** each ran the same script again for the next 45 largest remaining duplicates (90 more modules). **Wave 5** ran `--limit=250` and barreled the rest of the same-named twin set (**243 files**); the duplicate-root candidate count is now **0** for this pattern. Unit tests that mocked root `lib/*` while implementations import `lib/utils/*` or `lib/analytics/*` were pointed at the **canonical paths** (see `tests/lib/statsService.test.ts`, `aggregationService.test.ts`, `geminiClient.test.ts`). For new tests, mock the module path the implementation actually imports (or import from the canonical file). To re-run the barrel helper after new duplicates appear: `node scripts/barrel-wave2.mjs` (optional `--limit=N`), then `npm run typecheck` and `npx vitest run`.
