...
|-------|--------|-----------------------------------------------------------|
| **1 — Inventory & portfolio** | Cards, targets, goals, registry, notifications tied to holdings | **Canonical (imported by UI):** `lib/utils/goalPlannerService.ts`, `lib/core/setRegistryService.ts`, `lib/utils/notificationCenterService.ts`, `lib/analytics/prospectPipelineService.ts`, `lib/analytics/rebalancingAlertService.ts`. **`lib/useInventory.ts`** re-exports **`lib/utils/useInventory.ts`** (syncStore + same `cardx_*` keys). Legacy duplicates at repo root `lib/goalPlannerService.ts`, `lib/setRegistryService.ts`, etc. are not wired from components — migrate or delete when convenient. |
