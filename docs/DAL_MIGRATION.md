...
|-------|--------|-----------------------------------------------------------|
| **1 — Inventory & portfolio** | Cards, targets, goals, registry, notifications tied to holdings | `lib/goalPlannerService.ts`, `lib/setRegistryService.ts`, `lib/prospectPipelineService.ts`, `lib/notificationCenterService.ts`, `lib/rebalancingAlertService.ts`; hooks **`lib/utils/useInventory.ts`** / **`lib/useInventory.ts`** should stay aligned with **`useSupabaseInventory`** + DAL keys. |
| **2 — Billing & account** | Stripe-adjacent state, auth history, compliance | **`lib/authenticationService.ts`** (auth event history in localStorage → user-scoped JSON via `store` / Supabase); tax and compliance services with local persistence (`lib/taxHarvestService.ts`, `lib/hobbyIncomeService.ts`, `lib/complianceCenterService.ts`). |
| **3 — Trading & marketplace** | Deals, listings, negotiation, liquidity demos | `lib/dealRoomService.ts`, `lib/marketplaceService.ts`, `lib/dealFinderService.ts`, `lib/negotiationAnalytics.ts`, `lib/negotiationPlaybooks.ts`, `lib/AutonomousExecutionService.ts`, `lib/auctionSniperService.ts`, `lib/consignmentMarketService.ts`. |
