...
**Current gate (2026-08-17, Wave-4 whitelist expanded):** `statements 98.4` / `branches 91.5` / `functions 99` / `lines 99` across the explicit `vite.config.ts` include list (now includes Web Push client readiness, migration field diffs, holding-horizon, and offline queue summary helpers). Full **100% branches** is often impractical (Vite-inlined `import.meta.env`, unreachable defensive branches, ternary explosion).
